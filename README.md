...
## 1.1.0-advanced — Resilience Upgrade

- **DeepSeek Harness**: automatic retry (5x, exponential backoff) on NVIDIA NIM / provider 503, 502, 504, overload, capacity, rate-limit, timeouts and connection drops. Fresh process per attempt. Live retry status in chat and foreground notification.
- **Claude bridge**: same transient-error detection and clearer user-facing messages.
- Tasks no longer stop in the middle when the upstream provider hiccups.
- Version `1.1.0-advanced` (versionCode 6).
