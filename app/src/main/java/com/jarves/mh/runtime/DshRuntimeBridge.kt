...
class DshRuntimeBridge(
    ...
    override suspend fun startSession(projectId: String, projectSlug: String, projectKind: ProjectKind, prompt: String, conversationHistory: List<ChatMessage>, provider: ProviderProfile): String = withContext(Dispatchers.IO + NonCancellable) {
        ...
        runCatching {
            ...
            // Retry the whole dsh process on transient NVIDIA / provider errors
            ...
            val sdkResult = runWithTransientRetry(sessionId, projectSlug) { attempt ->
                ...
                Log.d("DshBridge", "SDK process exited with code $exit (attempt $attempt)")
                activeProcess = null
                if (userStopRequested) throw DshSessionException("Stopped by user")
                if (exit == 0 && result.completed) {
                    result
                } else {
                    val fail = result.failure.ifBlank { "DeepSeek Harness stopped with exit code $exit" }
                    // Return incomplete result so the retry helper can decide
                    DshSdkRunResult(completed = false, failure = fail)
                }
            }
            ...
            if (sdkResult.completed && !userStopRequested) {
                ...
                if (userStopRequested) throw DshSessionException("Stopped by user")
                error(sdkResult.failure.ifBlank { "DeepSeek Harness stopped before finishing the task" })
            }
        }.onFailure { error ->
    ...
    /**
     * Runs one full dsh process. On transient upstream errors (NVIDIA 503,
     * overload, gateway timeouts, etc.) retries with exponential backoff
     * instead of aborting the whole task mid-flight.
     */
    private suspend fun runWithTransientRetry(
        sessionId: String,
        projectSlug: String,
        maxAttempts: Int = TRANSIENT_MAX_RETRIES,
        block: suspend (attempt: Int) -> DshSdkRunResult,
    ): DshSdkRunResult {
        var lastFailure = ""
        var attempt = 0
        while (attempt < maxAttempts) {
            ...
            try {
                ...
                // process exited without completing — decide if retryable
                val failMsg = result.failure.ifBlank { "DeepSeek Harness stopped before finishing" }
                if (!isTransientProviderError(failMsg) || attempt >= maxAttempts) {
                    return DshSdkRunResult(completed = false, failure = failMsg)
                }
                lastFailure = failMsg
            } catch (e: Exception) {
                if (userStopRequested) throw e
                val msg = e.message.orEmpty()
                if (!isTransientProviderError(msg) || attempt >= maxAttempts) throw e
                lastFailure = msg
            }
            val backoff = (TRANSIENT_BACKOFF_BASE_MS * (1L shl (attempt - 1)))
                .coerceAtMost(TRANSIENT_BACKOFF_MAX_MS)
            Log.w(
                "DshBridge",
                "Transient provider error (attempt $attempt/$maxAttempts): $lastFailure — retrying in ${backoff}ms",
            )
            eventBus.emit(
                RuntimeEvent.AssistantDelta(
                    sessionId,
                    "\n[harness] Provider temporarily unavailable (503/overload). Retry $attempt/$maxAttempts in ${backoff / 1000}s…\n",
                ),
            )
            pushForegroundProgress("NVIDIA/provider busy — retry $attempt/$maxAttempts…")
            delay(backoff)
            if (userStopRequested) throw DshSessionException("Stopped by user")
        }
        return DshSdkRunResult(
            completed = false,
            failure = lastFailure.ifBlank { "Provider kept returning transient errors after $maxAttempts attempts" },
        )
    }
    ...
    private fun friendlyError(error: Throwable): String {
        ...
        return when {
            ...
            isTransientProviderError(message) ->
                "NVIDIA / provider is temporarily unavailable (HTTP 503 or overload). The harness will retry automatically."
    ...
    /** True for transient upstream failures that are safe (and useful) to retry. */
    private fun isTransientProviderError(message: String): Boolean {
