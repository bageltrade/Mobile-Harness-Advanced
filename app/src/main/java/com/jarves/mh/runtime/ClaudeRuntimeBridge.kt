...
internal object ProviderRuntimeErrorDetector {
    fun isTransient(line: String): Boolean {
        ...
        return listOf(
            ...
            "gateway timeout", "bad gateway", "upstream",
            "try again", "retry later", "server error",
            "nvidia", "nim",
        ).any { it in m }
    }
    ...
    fun detect(line: String): String? {
        ...
        return when {
            isTransient(combined) ->
                "Provider temporarily unavailable (HTTP 503/overload). Will retry automatically."
            ...
            "http 429" in combined || "rate limit" in combined ->
                "Provider rate-limited the request. Will retry automatically."
            else -> null
        }
    }
}
...
class ClaudeRuntimeBridge(
    ...
    private fun friendlyError(error: Throwable): String {
        ...
        return when {
            ...
            ProviderRuntimeErrorDetector.isTransient(message) ->
                "Provider temporarily unavailable (HTTP 503/overload). The harness will retry automatically."
